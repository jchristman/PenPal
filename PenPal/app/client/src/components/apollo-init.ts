...
const WS_RETRY_CONFIG = {
  ...
  maxDelay: 30000,
  backoffMultiplier: 1.5,
  shouldRetry: (errorOrCloseEvent: { code?: number } | CloseEvent | undefined) => {
...
const introspect_schema = async (
  ...
  try {
    ...
    result_json.data.__schema.types.forEach((supertype: any) => {
  ...
  try {
    ...
    const wsLink = new GraphQLWsLink(
      createClient({
        ...
        retryWait: async function (retries: number) {
          const delay = Math.min(
        ...
        on: {
          ...
          connected: (_socket: Client, _payload: unknown) => {
            console.log("✅ WebSocket connected successfully");
            setWebSocketState(WS_CONNECTION_STATES.CONNECTED);
          },
          message: ({ type }: { type: string; payload?: unknown }) => {
