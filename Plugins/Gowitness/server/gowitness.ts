...
import { GowitnessLogger } from "./plugin.ts";

interface ScreenshotScanOptions {
  project_id: string;
  http_services: any[];
  update_job?: (
    progress: number,
...
interface ScreenshotScanResult {
  ...
  enrichment_updates: EnrichmentUpdate[];
  message?: string;
  error?: string;
  statusText?: string;
}
    ...
    if (enrichment_updates.length > 0) {
      ...
      if (result.rejected?.length > 0) {
        GowitnessLogger.warn(
          ...
          result.rejected.map(
            (r: any) => `${r.selector.host}:${r.selector.port} - ${r.error}`
          )
        );
      }
      ...
      if (result.accepted?.length > 0) {
        const screenshots = result.accepted.map((accepted_result: any) => {
        ...
        if (PenPal.API && PenPal.API.MQTT && PenPal.API.MQTT.Publish) {
          const topic =
            ...
            "penpal/gowitness/new/screenshots";
          GowitnessLogger.log(
          ...
          GowitnessLogger.warn(
            "MQTT API not available, skipping screenshot event publication"
          );
        }
      }
...
export const performScreenshotScan = async (
  ...
  try {
    ...
    if (!http_services || http_services.length === 0) {
      ...
      return {
        success: false,
        message: "No HTTP services provided for screenshot scan",
          ...
          if (job_id) {
            ...
            if (currentJob?.cancellation_request) {
              try {
                ...
                GowitnessLogger.warn(
                  `Error stopping Gowitness container on cancellation: ${
              ...
              return {
                success: false,
                message: "Gowitness scan cancelled",
                screenshots_captured: 0,
                enrichment_updates: [],
    ...
    return {
      ...
      enrichment_updates: uploaded_screenshots,
    };
  } catch (error: unknown) {
    const message = error instanceof Error ? error.message : String(error);
    GowitnessLogger.error("Error in Gowitness scan:", error);
    await update_job(100, `Gowitness scan failed: ${message}`, "failed");
    ...
    if (container_id && job_id) {
      try {
        ...
        await PenPal.Jobs.Update(job_id, {
          ...
          stderr: logs.stderr || message || "",
        });
      } catch (logError: unknown) {
        GowitnessLogger.warn(
          `Failed to capture logs on error:`,
          logError instanceof Error ? logError.message : String(logError)
        );
      }
    }
