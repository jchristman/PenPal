...
import { DockerLogger as logger } from "./plugin.ts";
const exec = util.promisify(_exec);

interface BuildTrackingRecord {
  timestamp: string;
  contextHash: string;
}
...
interface BuildTracking {
  [imageName: string]: BuildTrackingRecord;
}
...
interface DockerComposeArgs {
  name: string;
  docker_compose_path: string;
}

interface DockerPullArgs {
  image: string;
}

interface DockerAttachArgs {
  container: string;
}
...
const loadBuildTracking = (): BuildTracking => {
  try {
...
const saveBuildTracking = (tracking: BuildTracking): void => {
  try {
...
const generateBuildContextHash = (args: DockerBuildArgs): string => {
...
const recordBuildAttempt = (args: DockerBuildArgs): void => {
...
const getLastBuildAttempt = (args: DockerBuildArgs): { timestamp: Date; contextHash: string } | null => {
  const tracking = loadBuildTracking();
...
const IsImageRecentlyBuilt = async (args: DockerBuildArgs): Promise<boolean> => {
...
const loadPullTracking = (): PullTracking => {
  try {
...
const savePullTracking = (tracking: PullTracking): void => {
  try {
    // Ensure directory exists
...
const recordPullAttempt = (image: string): void => {
...
const getLastPullAttempt = (image: string): Date | null => {
  const tracking = loadPullTracking();
...
export const WaitForImageReady = async (imageName: string, options: { timeout?: number; updateCallback?: (progress: number, message: string) => Promise<void>; updateMessage?: string } = {}): Promise<boolean> => {
  const {
...
const processDeferredJobs = async (): Promise<void> => {
  if (
    PenPal.DataStore &&
...
const startDeferredJobsTimer = (): void => {
  if (!DeferredJobs.timer) {
...
const safeJobOperation = async (operation: (...args: any[]) => any, ...args: any[]): Promise<any> => {
  try {
...
const check_volume = async (volume: { name: string; path: string }): Promise<{ stdout: string; stderr: string } | { name: string; path: string }> => {
...
export const Compose = async (args: DockerComposeArgs): Promise<void> => {
  await PenPal.Utils.AsyncNOOP();
...
export const Run = async (options: DockerRunOptions): Promise<{ stdout: string; stderr: string }> => {
  const {
    image,
    name,
    cmd = "",
...
export const Wait = async (container_id: string, timeout_ms: number | null = null): Promise<{ stdout: string; stderr: string }> => {
  await PenPal.Utils.AsyncNOOP();
...
export const Start = async (container_id: string): Promise<{ stdout: string; stderr: string }> => {
  await PenPal.Utils.AsyncNOOP();
  const output = await exec(`docker ${docker_host} start ${container_id}`);
  return output;
};

export const Exec = async (options: DockerExecOptions): Promise<{ stdout: string; stderr: string }> => {
  const { container, cmd } = options;
  await PenPal.Utils.AsyncNOOP();
  const output = await exec(`docker ${docker_host} exec ${container} ${cmd}`);
  return output;
};

export const Stop = async (container_id: string): Promise<{ stdout: string; stderr: string }> => {
  await PenPal.Utils.AsyncNOOP();
  const output = await exec(`docker ${docker_host} stop ${container_id}`);
  return output;
};

export const RemoveContainer = async (container_id: string): Promise<{ stdout: string; stderr: string }> => {
  await PenPal.Utils.AsyncNOOP();
  const output = await exec(`docker ${docker_host} rm ${container_id}`);
  return output;
};

export const Copy = async (options: DockerCopyOptions): Promise<{ stdout: string; stderr: string }> => {
  const { container, container_file, output_file } = options;
  await PenPal.Utils.AsyncNOOP();
  const output = await exec(
 ...
 */
export const Logs = async (container_id: string, options: DockerLogsOptions = {}): Promise<DockerLogsResult> => {
  await PenPal.Utils.AsyncNOOP();
...
export const Raw = async (cmd: string): Promise<string> => {
  await PenPal.Utils.AsyncNOOP();
...
export const IsImageRecentlyPulled = async (image: string): Promise<boolean> => {
  await PenPal.Utils.AsyncNOOP();
...
export const DoComposeImagesNeedPull = async (docker_compose_path: string): Promise<boolean> => {
  await PenPal.Utils.AsyncNOOP();
...
const recordComposePullAttempts = async (docker_compose_path: string): Promise<void> => {
  try {
    // Get list of images defined in compose file
    const configOutput = await exec(
      `docker ${docker_host} compose -f ${docker_compose_path} config --images`
...
export const Pull = async (options: DockerPullArgs): Promise<{ stdout: string; stderr: string }> => {
  const { image } = options;
  await PenPal.Utils.AsyncNOOP();
...
export const Build = async (args: DockerBuildArgs): Promise<{ stdout: string; stderr: string } | { stdout: string; stderr: string } | null> => {
  await PenPal.Utils.AsyncNOOP();
...
export const AttachAndReturnDockerChildProcess = async (args: DockerAttachArgs): Promise<any> => {
  await PenPal.Utils.AsyncNOOP();
...
export const DetachFromDockerChildProcess = async (term: any): Promise<void> => {
...
export const CreateTemporaryVolume = async (): Promise<{ name: string; output: { stdout: string; stderr: string } }> => {
  await PenPal.Utils.AsyncNOOP();
...
export const DeleteTemporaryVolume = async (name: string): Promise<{ stdout: string; stderr: string }> => {
  await PenPal.Utils.AsyncNOOP();
