...
import * as DockerImports from "./docker.ts";
...
const check_docker = (docker: DockerConfig): boolean => {
...
const build_docker_images = async (): Promise<void> => {
...
const build_docker_images_background = async (): Promise<void> => {
  const build_docker = async (docker: DockerConfig): Promise<void> => {
    if (docker) {
...
const DockerPlugin: PenPalPlugin = {
