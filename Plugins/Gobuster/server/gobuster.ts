...
import { GobusterLogger } from "./plugin.ts";

interface GobusterProgressStats {
  current: number;
  total: number;
  percentage: number;
}
...
const extractGobusterProgress = (output: string): GobusterProgressStats | null => {
...
const getGobusterProgress = async (container_id: string): Promise<{ stats: GobusterProgressStats | null; foundDirectories: FoundDirectory[] }> => {
  let stats = null;
...
export const performGobusterScan = async (options: GobusterScanOptions): Promise<void> => {
  const {
    http_services,
    project_id,
    update_job = () => {},
