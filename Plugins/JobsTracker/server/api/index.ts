...
import {
  ...
  COMPLETED_STATUSES,
  ACTIVE_STATUSES,
} from "../../common/job-constants.ts";

// Import the shared logger from plugin.ts
import { JobsTrackerLogger as logger } from "../plugin.ts";
...
const sanitizeJobForPubSub = (job: Job | null | undefined): Job | null => {
  if (!job) {
...
export const getJobs = async (job_ids: string[]): Promise<Job[]> => {
...
export const getJobsByPlugin = async (plugin_name: string): Promise<Job[]> => {
  return await PenPal.DataStore.fetch("JobsTracker", "Jobs", {
    plugin: plugin_name,
  });
};

export const insertJob = async (job: Omit<Job, 'id'>): Promise<Job> => {
...
export const insertJobs = async (jobs: Omit<Job, 'id'>[]): Promise<Job[]> => {
...
export const updateJob = async (job_id: string, updates: Partial<Job>): Promise<Job> => {
