...
import GetStagedAssets from "../queries/get-staged-assets.js";
import AcceptStagedAssets from "../mutations/accept-staged-assets.js";
import RejectStagedAssets from "../mutations/reject-staged-assets.js";
...
const AutoReconSection = ({ project }) => {
  ...
  const { data: jobsData, loading: jobsLoading, refetch: refetchJobs } = useQuery(gql`
  ...
  const { data: assetsData, loading: assetsLoading, refetch: refetchAssets } = useQuery(GetStagedAssets, {
  ...
  const autoreconJobs = allJobs.filter(job => job.plugin === 'AutoRecon' && job.project_id === project.id);
  const stagedAssets = assetsData?.getStagedAssets || [];
  const activeJob = autoreconJobs.find(job => job.status === 'running' || job.status === 'pending');
    ...
    try {
      ...
      if (result.data?.acceptStagedAssets) {
        ...
        toast({
          ...
          description: `Successfully added ${accepted} assets to project scope. ${rejected > 0 ? `${rejected} failed.` : ''}`,
        });
        setSelectedAssetIds([]);
        refetchAssets();
      }
  ...
  return (
    <div className="border-t pt-6 mt-6">
      <div className="flex items-center justify-between mb-4">
        <div>
          <h3 className="text-lg font-semibold">AutoRecon - Automated Subdomain Discovery</h3>
          <p className="text-sm text-gray-600">
            Discover subdomains that may be in scope using multiple enumeration tools in a containerized environment
          </p>
        </div>
        <div className="flex gap-2">
          ...
          >
                        {scanInProgress ? "Starting..." : activeJob ? "Scan In Progress" : "Start AutoRecon"}
          </button>
        </div>
      </div>
      ...
      {stagedAssets.length > 0 && (
        <div>
            ...
            {stagedAssets.map((asset) => (
              ...
              >
                ...
                <span className="text-lg">
                  {asset.type === 'domain' ? '🌐' : asset.type === 'host' ? '🖥️' : '📄'}
                </span>
          ...
          <p className="text-xs text-gray-500 mt-2">
            Select subdomains above to add them to your project scope as domains. Only selected subdomains will be included.
          </p>
        </div>
      )}
                  ...
                  {stagedAssets.length === 0 && !activeJob && autoreconJobs.filter(j => j.status === 'done').length === 0 && (
        <div className="text-center py-8 text-gray-500">
          <p>No staged subdomains. Start an AutoRecon scan to discover potential subdomains in scope.</p>
        </div>
      )}
    </div>
  );
