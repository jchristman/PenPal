...
interface MongoAdapterType {
  MongoCollections: Record<string, Collection>;
  client: MongoClient | null;
  db?: Db;
  connect: () => Promise<void>;
  CreateStore?: (plugin_name: string, store_name: string) => Promise<Collection>;
  DeleteStore?: (plugin_name: string, store_name: string) => Promise<any>;
  fetch?: (plugin_name: string, store_name: string, selector: any, options?: any) => Promise<any[]>;
  getPaginationInfo?: (plugin_name: string, store_name: string, selector: any, options?: any) => Promise<any>;
  fetchOne?: (plugin_name: string, store_name: string, selector: any, options?: any) => Promise<any>;
  insert?: (plugin_name: string, store_name: string, data: any) => Promise<string>;
  insertMany?: (plugin_name: string, store_name: string, data: any[]) => Promise<any[]>;
  updateOne?: (plugin_name: string, store_name: string, selector: any, data: any) => Promise<any>;
  updateMany?: (plugin_name: string, store_name: string, selector: any, data: any) => Promise<any>;
  delete?: (plugin_name: string, store_name: string, selector: any) => Promise<any>;
  isReady?: () => Promise<boolean>;
}

const MongoAdapter: MongoAdapterType = {} as MongoAdapterType;
MongoAdapter.MongoCollections = {};
MongoAdapter.client = null;

MongoAdapter.connect = async (): Promise<void> => {
...
const collection_name = (plugin_name: string, store_name: string): string =>
  `${plugin_name}.${store_name}`;

const get_collection = (plugin_name: string, store_name: string): Collection =>
  MongoAdapter.MongoCollections[collection_name(plugin_name, store_name)];

const normalize_data = ({ id = null, ...rest }: any = {}): any => {
  return { ...(id !== null && { _id: id }), ...rest };
};

const normalize_result = ({ _id = null, ...rest }: any = {}): any => {
  return { ...(_id !== null && { id: String(_id) }), ...rest };
};

const check_options = (options: any): void => {
  ...
  check(sort, (x: number) => x === 1 || x === -1);
};

// -----------------------------------------------------------------------
// MongoAdapter creation/deletion functions

MongoAdapter.CreateStore = async (plugin_name: string, store_name: string): Promise<Collection> => {
...
MongoAdapter.DeleteStore = async (plugin_name: string, store_name: string): Promise<any> => {
...
MongoAdapter.fetchOne = async (plugin_name: string, store_name: string, selector: any, options?: any): Promise<any> => {
...
MongoAdapter.insert = async (plugin_name: string, store_name: string, data: any): Promise<string> => {
...
MongoAdapter.insertMany = async (plugin_name: string, store_name: string, data: any[] = []): Promise<any[]> => {
  ...
  const results = await get_collection(plugin_name, store_name).insertMany(
    data.map((datum: any) => normalize_data(datum))
  );

  return (
    Object.values(results.insertedIds)?.map((object_id: any) => ({
      id: String(object_id),
    })) ?? []
  );
};

MongoAdapter.updateOne = async (plugin_name: string, store_name: string, selector: any, data: any): Promise<any> => {
  // MongoDB requires atomic operators for updates. If data doesn't contain operators, wrap in $set
  const updateDoc =
    data &&
    typeof data === "object" &&
    !Object.keys(data).some((key: string) => key.startsWith("$"))
      ? { $set: data }
      : data;
...
MongoAdapter.updateMany = async (plugin_name: string, store_name: string, selector: any, data: any): Promise<any> => {
  // MongoDB requires atomic operators for updates. If data doesn't contain operators, wrap in $set
  const updateDoc =
    data &&
    typeof data === "object" &&
...
MongoAdapter.delete = async (plugin_name: string, store_name: string, selector: any): Promise<any> => {
...
MongoAdapter.isReady = async (): Promise<boolean> => {
  try {
