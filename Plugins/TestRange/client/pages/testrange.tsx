...
import GetRunningContainers from "./testrange/queries/get-running-containers.ts";
import GetAvailableContainers from "./testrange/queries/get-available-containers.ts";
import GetRecentContainers from "./testrange/queries/get-recent-containers.ts";
import StartContainerMutation from "./testrange/mutations/start-container.ts";
import StopContainerMutation from "./testrange/mutations/stop-container.ts";
import RemoveContainerMutation from "./testrange/mutations/remove-container.ts";
import RestartContainerMutation from "./testrange/mutations/restart-container.ts";
import DeployVulhubContainerMutation from "./testrange/mutations/deploy-vulhub-container.ts";

const { useToast } = Hooks;

const TestRange: React.FC = () => {
  ...
  const handleDeploy = async (containerPath: string, containerName: string) => {
    try {
            ...
            <div className="bg-white rounded-lg shadow overflow-hidden">
              <table className="min-w-full divide-y divide-gray-200">
                ...
                <tbody className="bg-white divide-y divide-gray-200">
                  {runningContainers.map((container: any) => (
                    <tr key={container.id}>
                      ...
                      <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                        {container.portMappings && container.portMappings.length > 0 ? (
                          <div className="space-y-1">
                            {container.portMappings.map((pm: any, idx: number) => (
