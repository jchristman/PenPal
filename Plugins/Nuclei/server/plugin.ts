...
import * as Nuclei from "./nuclei.ts";
import { severitiesToArray } from "./graphql/resolvers/nuclei.config.ts";
 ...
 */
const getEffectiveNucleiConfig = async (project_id: string): Promise<NucleiConfiguration> => {
  try {
 ...
 */
const isNucleiEnabled = async (project_id: string): Promise<boolean> => {
  try {
...
const start_nuclei_scan_batch = async (batchedArgs: BatchArgs): Promise<void> => {
...
const NucleiPlugin: PenPalPlugin = {
