...
type ComponentMap = Record<ComponentMapKey, any>;
...
const renderField = (fieldName: string, fieldData: any, fieldConfig: any) => {
  ...
  const componentType = type as ComponentMapKey;
...
const DirectiveRenderer: React.FC<DirectiveRendererProps> = ({
  ...
  const uiConfig: UIConfig = directiveData?.getUIDirectives;

  // Normalize uiGroup to always be an array
  let uiGroups: UIGroup[] | undefined;
  const rawUiGroups = uiConfig.typeDirectives?.uiGroup;
  if (rawUiGroups) {
    uiGroups = Array.isArray(rawUiGroups) ? rawUiGroups : [rawUiGroups];
  }
  ...
  const fieldsByGroup = Array.from(allFields)
    .filter((fieldName) => {
      ...
      if (
        ...
        uiConfig.fields["confidence_scores_table"]?.uiComponent
      ) {
        return false;
      }
    ...
    .reduce((acc: Record<string, string[]>, fieldName) => {
      const groupName =
        ...
        return (
          <div key={groupName} className="mb-2">
            ...
            {groupFields.map((fieldName) => {
              ...
              if (
                fieldName === "confidence_scores_table" &&
              ...
              if (
                ...
                if (ScreenshotImage) {
                  const label =
                    fieldConfig.uiComponent.config?.label ||
                    fieldName.replace(/_/g, " ");
                  return (
                    <div key={fieldName} className="mb-1">
                  ...
                  console.warn(
                    "[DirectiveRenderer] ScreenshotImage component not found. Bucket:",
                    data.screenshot_bucket,
                    "Key:",
                    data.screenshot_key
                  );
                  return (
                    <div key={fieldName} className="mb-1">
                      ...
                      <p className="text-sm text-muted-foreground">
                        Screenshot available (bucket: {data.screenshot_bucket},
                        key: {data.screenshot_key})
                      </p>
                    </div>
                  );
                }
