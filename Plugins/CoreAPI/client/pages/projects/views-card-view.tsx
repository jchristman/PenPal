...
const StatBadge = ({ icon: Icon, label, value }: { icon: React.ComponentType<any>; label: string; value: string | number }) => (
...
const ProjectCard = ({ project }: { project: Project }) => {
  ...
  const handleNavigate = (project_id: string) => {
  ...
  const projectDuration = (start: string, end: string) => {
      ...
      <CardContent className="flex-grow flex flex-col justify-between space-y-4">
        <div className="space-y-3 text-sm">
          <div className="flex items-center space-x-2">
            ...
            <span className="font-medium">{project.customer?.name || "Not assigned"}</span>
          </div>
          <div className="flex items-center space-x-2">
        ...
        <div className="grid grid-cols-3 gap-x-3 gap-y-3 pt-4 border-t border-border/50">
          <StatBadge
            icon={GlobeAltIcon}
            label="Networks"
            value={project.stats?.networks || 0}
          />
          <StatBadge
            icon={ComputerDesktopIcon}
            label="Hosts"
            value={project.stats?.totalHosts || 0}
          />
          <StatBadge
            icon={ServerIcon}
            label="Services"
            value={project.stats?.totalServices || 0}
          />
        </div>
      </CardContent>
    </Card>
