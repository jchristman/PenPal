...
const SeverityBadge = ({ severity }) => {
  ...
  return (
    <Badge
      variant={variantMap[severity] || "outline"}
      className={`text-xs font-semibold ${colorMap[severity] || ""}`}
    >
      {severity}
    </Badge>
  );
...
const DiscoveredByBadge = ({ discoveredBy }) => {
...
const ProjectViewVulnerabilitiesTable = ({
  ...
  const [sort, setSort] = useState({ key: "severity", direction: "desc" });
    ...
    if (debouncedSearchTerm) {
      ...
      filtered = filtered.filter(
        (v) =>
          ...
          v.cveIds?.some((cve) => cve.toLowerCase().includes(term)) ||
  ...
  const sortedVulnerabilities = useMemo(() => {
    ...
    return [...filteredVulnerabilities].sort((a, b) => {
      ...
      if (sort.key !== "cvssScore" && sort.key !== "discoveredAt") {
  ...
  const formatAffectedHosts = (hosts) => {
