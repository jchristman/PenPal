...
const ProjectViewDomainsDashboard = ({ domains = [] }: { domains?: Domain[] }) => {
  const stats = useMemo(() => {
    const totalDomains = domains.length;
    const resolvedDomains = domains.filter((d) => (d.resolved_ips?.length ?? 0) > 0).length;
    ...
    const resolutionDistribution = domains.reduce((acc: Record<string, number>, domain) => {
    ...
    const tldDistribution = domains.reduce((acc: Record<string, number>, domain) => {
