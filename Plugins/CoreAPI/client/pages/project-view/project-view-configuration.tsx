...
interface ProjectViewConfigurationProps {
  project: Project;
...
const ProjectViewConfiguration: React.FC<ProjectViewConfigurationProps> = ({ project, disable_polling }) => {
  ...
  } = useQuery(PenPal.Profiles.GetProfiles, {
  ...
  const currentProfile = profiles.find((p: any) => p.id === currentProject?.profile);
  ...
  const renderProfileSummary = (profile: any) => {
    if (!profile) {
      return (
        ...
        {profile.plugin_configs && profile.plugin_configs.length > 0 ? (
          <div className="space-y-3">
            ...
            <div className="grid gap-3">
              {profile.plugin_configs.map((config: any, index: number) => {
