...
const ProjectViewNetworksTable = ({ networks = [] }: { networks?: Network[] }) => {
  ...
  const getTotalServices = (network: Network) => {
    if (!network?.hostsConnection?.hosts) return 0;
    return network.hostsConnection.hosts.reduce(
      (acc: number, host: any) => acc + (host.servicesConnection?.totalCount || 0),
      0
    );
  };
