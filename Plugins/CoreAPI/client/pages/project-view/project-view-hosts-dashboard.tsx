...
const ProjectViewHostsDashboard = ({ hosts = [] }: { hosts?: Host[] }) => {
  const stats = useMemo(() => {
    const totalHosts = hosts.length;
    const hostsWithNames = hosts.filter((h) => (h.domains?.length ?? 0) > 0).length;
    ...
    // OS distribution
    const osDistribution = hosts.reduce((acc: Record<string, number>, host) => {
    ...
    const serviceDistribution = hosts.reduce((acc: Record<string, number>, host) => {
