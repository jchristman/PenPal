...
import GetNetworksInformation from "./queries/get-networks-information.ts";
import GetHostsInformation from "./queries/get-hosts-information.ts";
import { parseScopeInput } from "../projects/scope-parser.ts";
import {
  PlusIcon,
  EyeIcon,
...
import GET_DOMAINS_BY_PROJECT from "./queries/get-domains-by-project.ts";
...
const ProjectViewDetails: React.FC<ProjectViewDetailsProps> = ({
  project,
  disable_polling,
}) => {
  ...
  // State for sorting
  const [networksSort, setNetworksSort] = useState<SortState>({
    key: "subnet",
    direction: "asc",
  });
  const [hostsSort, setHostsSort] = useState<SortState>({
    key: "ip_address",
    direction: "asc",
  });
  const [domainsSort, setDomainsSort] = useState<SortState>({
    key: "name",
    direction: "asc",
  });
  ...
  const filteredAndSortedNetworks = useMemo(() => {
    ...
    if (networksSort.key) {
      filtered = [...filtered].sort((a, b) => {
        ...
        // For string-based sorting (subnet and domain)
  ...
  const filteredAndSortedHosts = useMemo(() => {
    let filtered = hosts.filter((host: any) => host && host.id);
      ...
      filtered = [...filtered].sort((a, b) => {
        ...
        switch (hostsSort.key) {
          case "ip_address":
            ...
            break;
          case "domains":
            aVal = a.domains?.filter((d: any) => d?.name)?.length || 0;
            bVal = b.domains?.filter((d: any) => d?.name)?.length || 0;
  ...
  const filteredAndSortedDomains = useMemo(() => {
    let filtered = domains.filter(
      (domain: any) => domain && domain.id && domain.name
    );
    ...
    if (domainsSearch.trim()) {
      ...
      filtered = filtered.filter(
        (domain: any) =>
          domain.name.toLowerCase().includes(searchTerm) ||
          domain.resolved_ips?.some((ip: string) =>
    ...
    if (domainsSort.key) {
      filtered = [...filtered].sort((a, b) => {
        ...
        // For string-based sorting (name and status)
        const result = aVal.localeCompare(bVal);
  ...
  const handleNetworksSort = (key: string) => {
  ...
  const handleHostsSort = (key: string) => {
  ...
  const handleDomainsSort = (key: string) => {
  ...
  const handleBulkParse = async () => {
    ...
    try {
      ...
      const existingIPs = new Set(hosts.map((h: any) => h.ip_address));
      const existingNetworks = new Set(networks.map((n: any) => n.subnet));
        ...
        <Components.CardHeader className="flex flex-row items-center justify-between">
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="flex items-center gap-2">
              ...
              {PenPal.ProjectScopeButtons.map(
                (buttonConfig: any, index: number) => {
          ...
          <div>
              ...
              <div className="flex items-center gap-4">
                ...
                <Components.Input
                  ...
                  onChange={(e: React.ChangeEvent<HTMLInputElement>) =>
                    setNetworksSearch(e.target.value)
                  }
                  className="w-64"
                />
              </div>
            ...
            {filteredAndSortedNetworks.length > 0 ? (
              <Table className="w-full">
                    ...
                    <TableHead className="w-12">
                      <Components.Checkbox
                        checked={
                          ...
                          filteredAndSortedNetworks.every((n: any) =>
                        ...
                        indeterminate={
                          ...
                          selectedNetworkIds.length <
                            filteredAndSortedNetworks.length &&
                          filteredAndSortedNetworks.some((n: any) =>
                            selectedNetworkIds.includes(n.id)
                          )
                        }
                        onCheckedChange={(checked: boolean) => {
                          if (checked) {
                            setSelectedNetworkIds(
                              filteredAndSortedNetworks.map((n: any) => n.id)
                            );
                          } else {
                            setSelectedNetworkIds([]);
                          }
                ...
                <TableBody>
                  {filteredAndSortedNetworks.map((network: any) => (
                    <TableRow key={network.id}>
                      <TableCell>
                        <Components.Checkbox
                          ...
                          onCheckedChange={(checked: boolean) => {
                            if (checked) {
                              setSelectedNetworkIds((prev: any) => [
                                ...prev,
                                network.id,
                              ]);
                            } else {
          ...
          <div>
            ...
            {filteredAndSortedHosts.length > 0 ? (
              <Table className="w-full">
                    ...
                    <TableHead className="w-12">
                      <Components.Checkbox
                        checked={
                          ...
                          filteredAndSortedHosts.every((h: any) =>
                        ...
                        indeterminate={
                          ...
                          selectedHostIds.length <
                            filteredAndSortedHosts.length &&
                          filteredAndSortedHosts.some((h: any) =>
                            selectedHostIds.includes(h.id)
                          )
                        }
                        onCheckedChange={(checked: boolean) => {
                          if (checked) {
                            setSelectedHostIds(
                              filteredAndSortedHosts.map((h: any) => h.id)
                            );
                          } else {
                            setSelectedHostIds([]);
                          }
                ...
                <TableBody>
                  {filteredAndSortedHosts.map((host: any) => (
                    <TableRow
                      key={host.id}
                      ...
                      <TableCell>
                        {host.domains && host.domains.length > 0 ? (
                          <div className="flex flex-wrap gap-1">
                            {host.domains
                              .slice(0, 2)
                              .filter((domain: any) => domain && domain.name)
            ...
            <div className="flex items-center justify-between mb-4">
              <div className="flex items-center gap-4">
                ...
                <Components.Input
                  ...
                  value={domainsSearch}
                  onChange={(e: React.ChangeEvent<HTMLInputElement>) =>
                    setDomainsSearch(e.target.value)
                  }
                  className="w-64"
                />
              </div>
                    ...
                    <TableHead className="w-12">
                      <Components.Checkbox
                        checked={
                          ...
                          filteredAndSortedDomains.every((d: any) =>
                        ...
                        indeterminate={
                          ...
                          selectedDomainIds.length <
                            filteredAndSortedDomains.length &&
                          filteredAndSortedDomains.some((d: any) =>
                            selectedDomainIds.includes(d.id)
                          )
                        }
                        onCheckedChange={(checked: boolean) => {
                          if (checked) {
                            setSelectedDomainIds(
                              filteredAndSortedDomains.map((d: any) => d.id)
                            );
                          } else {
                            setSelectedDomainIds([]);
                          }
...
const HostDetailsDrawer: React.FC<HostDetailsDrawerProps> = ({
  host,
  isOpen,
  onClose,
}) => {
  ...
  const allDomains = host.domains?.filter((d: any) => d && d.name) || [];
...
interface StagedAssetsPanelProps {
  ...
  onAssetSelectionChange: (ids: string[] | ((prev: string[]) => string[])) => void;
  onAcceptAssets: () => void;
  onRejectAssets: () => void;
  onClose: () => void;
}

const StagedAssetsPanel: React.FC<StagedAssetsPanelProps> = ({
  stagedAssets,
  selectedAssetIds,
  ...
  const [sort, setSort] = useState<SortState>({ key: "value", direction: "asc" });
  ...
  const sortedAssets = useMemo(() => {
    ...
    return [...filteredAssets].sort((a, b) => {
      ...
      // For string-based sorting (value, tool, and type)
        const result = aVal.localeCompare(bVal);
  ...
  const handleSelectAllFiltered = () => {
    ...
    if (allSelected) {
      ...
      onAssetSelectionChange((currentSelected: string[]) =>
        currentSelected.filter((id: string) => !filteredIds.includes(id))
      );
    } else {
      // Select all filtered
      onAssetSelectionChange((currentSelected: string[]) => [
                ...
                {sortedAssets.map((asset) => (
                  <Components.TableRow
                    ...
                    onClick={() =>
                      onAssetSelectionChange((currentSelected: string[]) =>
                        selectedAssetIds.includes(asset.id)
                          ? currentSelected.filter((id: string) => id !== asset.id)
