...
const ProjectViewHostsGraph = ({ hosts = [] }: { hosts?: Host[] }) => {
  const svgRef = useRef<SVGSVGElement>(null);
  const [selectedHost, setSelectedHost] = useState<(Node & { x: number; y: number; groupName: string }) | null>(null);
  const [dimensions, setDimensions] = useState({ width: 800, height: 600 });

  // Prepare grid data organized by service count groups
  const gridData = useMemo(() => {
    const nodes: Node[] = hosts.map((host) => ({
      id: host.id,
    ...
    const positionedNodes: (Node & { x: number; y: number; groupName: string })[] = [];
  ...
  useEffect(() => {
    ...
    const node = zoomContainer
      ...
      .join("g")
      .attr("cursor", "pointer")
      .attr("transform", (d: Node & { x: number; y: number; groupName: string }) => `translate(${d.x},${d.y})`);
    ...
    node
      ...
      .attr("r", 15)
      .attr("fill", (d: Node & { x: number; y: number; groupName: string }) => {
    ...
    node
      .append("text")
      .text((d: Node & { x: number; y: number; groupName: string }) => d.ip_address)
      .attr("x", 0)
      .attr("y", -25)
    ...
    node
      .append("text")
      .text((d: Node & { x: number; y: number; groupName: string }) => d.serviceCount)
      .attr("x", 12)
      .attr("y", -8)
      .attr("text-anchor", "middle")
      .attr("font-size", "8px")
    ...
    // Click handler
    node.on("click", (event: any, d: Node & { x: number; y: number; groupName: string }) => {
  ...
  useEffect(() => {
    d3.select(svgRef.current)
      ...
      .transition()
      .duration(200)
      .attr("stroke", (d: Node & { x: number; y: number; groupName: string }) => (d.id === selectedHost?.id ? "#3b82f6" : "#fff"))
      .attr("stroke-width", (d: Node & { x: number; y: number; groupName: string }) => (d.id === selectedHost?.id ? 4 : 2));
  }, [selectedHost]);
