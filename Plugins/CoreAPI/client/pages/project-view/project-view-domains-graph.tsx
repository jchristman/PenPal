...
interface DomainNode extends Domain {
  ipCount: number;
  x: number;
  y: number;
  groupName: string;
...
const ProjectViewDomainsGraph = ({ domains = [] }: { domains?: Domain[] }) => {
  ...
  const gridData = useMemo(() => {
    const nodes: Array<Omit<DomainNode, 'x' | 'y' | 'groupName' | 'group'>> = domains.map((domain) => ({
      id: domain.id,
      name: domain.name,
    ...
    const groups: Record<string, Omit<DomainNode, 'x' | 'y' | 'group' | 'groupName'>[]> = {
      resolved: nodes
    ...
    const positionedNodes: DomainNode[] = [];
  ...
  useEffect(() => {
    ...
    const node = zoomContainer
      ...
      .join("g")
      .attr("cursor", "pointer")
      .attr("transform", (d: DomainNode) => `translate(${d.x},${d.y})`);
    ...
    node
      ...
      .attr("height", 50)
      .attr("rx", 8)
      .attr("fill", (d: DomainNode) => (d.hasResolution ? "#dcfce7" : "#fef2f2"))
      .attr("stroke", (d: DomainNode) => (d.hasResolution ? "#22c55e" : "#ef4444"))
      .attr("stroke-width", 2);
    ...
    // Click handler
    node.on("click", (event: any, d: DomainNode) => {
  ...
  useEffect(() => {
    d3.select(svgRef.current)
      ...
      .transition()
      .duration(200)
      .attr("stroke-width", (d: DomainNode) => (d.id === selectedDomain?.id ? 4 : 2));
  }, [selectedDomain]);
          ...
          <div className="absolute top-4 left-4 w-80 z-10">
            <Card>
              ...
              <CardContent className="space-y-3">
                ...
                {/* Resolved IPs */}
                {selectedDomain.resolved_ips && selectedDomain.resolved_ips.length > 0 && (
                  <div>
