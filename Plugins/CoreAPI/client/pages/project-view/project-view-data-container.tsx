...
PenPal.API.registerProjectViewTab = (tabDescriptor: any) => {
...
PenPal.API.registerProjectScopeSection = (sectionDescriptor: any) => {
  const { component, order = 100 } = sectionDescriptor || {};
  if (!component) return;
  PenPal.API.ProjectScopeSectionsRegistry.push({ component, order });
};

interface ProjectViewDataContainerProps {
  project: any;
  disable_polling?: boolean;
}

const ProjectViewDataContainer: React.FC<ProjectViewDataContainerProps> = ({ project, disable_polling }) => {
