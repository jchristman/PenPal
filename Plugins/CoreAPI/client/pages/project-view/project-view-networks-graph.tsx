...
const ProjectViewNetworksGraph = ({ networks = [] }: { networks?: Network[] }) => {
  ...
  useEffect(() => {
    ...
    const simulation = d3
      ...
      .force(
        "collide",
        d3.forceCollide().radius((d: any) => (d.type === "network" ? 55 : 40))
      );
    ...
    node
      ...
      .attr("r", (d: any) => (d.type === "network" ? 20 : 15))
      .attr("fill", (d: any) => {
        if (d.type === "network") {
    ...
    node
      ...
      .attr("dy", (d: any) => (d.type === "network" ? "40" : "30")) // Position below the circle
      .attr("font-size", (d: any) => (d.type === "network" ? "14px" : "12px")) // Increased from 11px/9px
    ...
    const hostNodes = node.filter((d: any) => d.type === "host");
    ...
    node
      .append("title")
      .text((d: any) => (d.type === "network" ? d.data.subnet : d.data.ip_address));
  ...
  useEffect(() => {
    d3.select(svgRef.current)
      ...
      .transition()
      .duration(200)
      .attr("stroke", (d: any) => (d.id === selectedNode?.id ? "#2563eb" : "#fff"))
                  ...
                  <div className="space-y-2">
                    ...
                    {selectedNode.data.domains?.length > 0 && (
                      <div>
                        ...
                        <div className="text-sm text-muted-foreground">
                          {selectedNode.data.domains.map((d: any) => d.name).join(", ")}
                        </div>
                      </div>
                    )}
                    <div>
