...
const DefaultEnrichmentCard = ({ service, enrichment }) => {
  return (
...
const ProjectViewServicesEnrichments = ({ services }) => {
  ...
  const enrichmentTypes = useMemo(() => {
    ...
    services.forEach((service) => {
      service.enrichments?.forEach((enrichment) => {
        ...
        if (!typeData.services.find((s) => s.id === service.id)) {
  ...
  const enrichmentsOfType = useMemo(() => {
    if (!selectedEnrichmentType) return [];
    const result = [];
    servicesWithEnrichment.forEach((service) => {
      service.enrichments?.forEach((enrichment) => {
  ...
  const handleEnrichmentTypeClick = (pluginName) => {
