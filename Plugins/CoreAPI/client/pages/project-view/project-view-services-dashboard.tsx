...
const ProjectViewServicesDashboard = ({ services = [] }: { services?: Service[] }) => {
  const stats = useMemo(() => {
    ...
    // Port distribution
    const portDistribution = services.reduce((acc: Record<string, number>, service) => {
    ...
    const topPorts = Object.entries(
      services.reduce((acc: Record<number, number>, service) => {
        if (service.port) {
    ...
    const serviceNames = services.reduce((acc: Record<string, number>, service) => {
    ...
    const pluginStats = services.reduce((acc: Record<string, number>, service) => {
        ...
        <Card>
          ...
          <CardContent className="space-y-3">
            {Object.entries(stats.pluginStats).map(([plugin, count]) => {
              const percentage = (((count as number) / stats.totalServices) * 100).toFixed(
                1
              );
              return (
