...
import { NmapLogger as logger } from "./plugin.ts";
...
interface ScanOptions {
  ...
  fast_scan?: boolean;
  update_job?: (progress: number, statusText: string, currentStage?: number | null) => Promise<void>;
  job_id?: string | null;
}

const extractNmapStats = (output: string): NmapStats | null => {
  const statsRegex =
...
const getNmapProgress = async (container_id: string): Promise<{ stats: NmapStats | null; hosts: any[] }> => {
  let stats = null;
...
export const performScan = async (options: ScanOptions): Promise<void> => {
  const {
    project_id,
    ips = [],
    networks = [],
