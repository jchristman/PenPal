...
import { EyeballerLogger } from "./plugin.ts";
...
interface ClassifyScreenshotOptions {
  screenshot_path: string;
  weights_path?: string;
  output_dir: string;
}

interface ProcessScreenshotsBatchOptions {
  project_id: string;
  screenshots: Screenshot[];
  update_job: (progress: number, statusText: string, status?: string) => Promise<void>;
}

interface EyeballerResult {
  screenshot_key: string;
  result: any;
  error?: string;
}
 ...
 */
const downloadScreenshot = async (bucket: string, key: string, local_path: string): Promise<boolean> => {
  try {
 ...
 */
const classifyScreenshot = async (options: ClassifyScreenshotOptions): Promise<any> => {
  const {
    screenshot_path,
 ...
 */
export const processScreenshot = async (options: {
  project_id: string;
  service_id: string;
 ...
 */
export const processScreenshotsBatch = async (options: ProcessScreenshotsBatchOptions): Promise<{
  total: number;
  successful: number;
  failed: number;
  results: EyeballerResult[];
}> => {
  const {
    project_id,
    screenshots,
